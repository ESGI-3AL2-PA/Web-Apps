/**
 * Suite de tests du cas d'usage desktop-authorize (étape /authorize du SSO desktop).
 *
 * Vérifie l'émission du code d'autorisation : seuls admin/superAdmin actifs (non bannis,
 * e-mail vérifié) obtiennent un code, stocké haché et lié à la requête (redirect_uri,
 * code_challenge). Contrôle aussi la résolution de session par hash sans rotation, le
 * comportement prompt=login (révocation serveur + unauthenticated) et l'unicité des codes.
 */
...
describe("desktopAuthorizeUseCase", () => {
  ...
  // Un admin obtient un code, stocké haché et lié à la requête (redirect_uri, challenge).
  it("issues a code for an admin and stores it hashed and bound to the request", async () => {
    ...
    // Le code brut ne doit jamais être persisté — seulement son digest.
  ...
  // Un superAdmin obtient aussi un code.
  ...
  // Le verrou pour lequel tout ce flux existe : un non-admin n'a rien à échanger.
  ...
  // Refuse les comptes bannis et à l'e-mail non vérifié (avec la raison correspondante).
  ...
  // Session absente, introuvable, expirée, ou utilisateur nul => unauthenticated.
  ...
  // Résout la session par hash et NE la fait PAS tourner.
  it("looks the session up by hash, and does not rotate it", async () => {
    const { run, refreshRepo } = setup();

    await run(input);

    // Faire tourner le token ici invaliderait le cookie détenu par l'onglet navigateur
    // d'où vient l'utilisateur — un login desktop ne doit pas le déconnecter de l'app web.
  ...
  // Sans ceci, le client desktop ne pourrait jamais changer de compte : le logout ne
  // vide que l'état côté JVM, donc le cookie navigateur ré-autorise le même utilisateur
  // instantanément.
  describe("prompt=login", () => {
    // Refuse une session valide et n'émet aucun code (force la ré-authentification).
    ...
    // Révoque la session côté serveur, pas seulement la copie navigateur.
    ...
    // Ne révoque rien lors d'un authorize normal (sans prompt=login).
  ...
  // Émet un code distinct à chaque appel.
