...
export default function IncidentsList() {
  ...
  const columns: Column<IncidentResponseDto>[] = [
    { header: t("incidents.col.category"), cell: (i) => i.category },
    {
      header: t("incidents.col.description"),
      cell: (i) => <span className="line-clamp-1 max-w-xs">{i.description}</span>,
    },
    { header: t("incidents.col.status"), cell: (i) => <StatusBadge value={i.status} /> },
    { header: t("incidents.col.assigned"), cell: (i) => <UserName id={i.assignedTo} /> },
    { header: t("incidents.col.created"), cell: (i) => formatDate(i.createdAt) },
  ];
    ...
    <div className="space-y-2">
      ...
      <Toolbar
        ...
        filters={[
          {
            key: "status",
            label: t("incidents.statusFilter"),
  ...
  return (
    <label className="input input-sm max-w-[12rem]">
      ...
      <input
        ...
        aria-label={t("incidents.categoryFilterAria")}
  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 gap-3 text-sm bg-base-200/50 rounded-box p-3">
        <div>
          <p className="text-xs text-base-content/50">{t("incidents.category")}</p>
          <p>{incident.category}</p>
        </div>
        <div>
          <p className="text-xs text-base-content/50">{t("incidents.reporter")}</p>
          <p>
            <UserName id={incident.reporterId} />
          </p>
        </div>
        <div className="col-span-2">
          <p className="text-xs text-base-content/50">{t("incidents.description")}</p>
          <p>{incident.description}</p>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <Field label={t("incidents.status")}>
